...
class ClHttp
{
     ...
     * @throws Exception
     */
    public static function downloadForClient($file_absolute, $show_name = '', $content = '', $expire = 180)
    {
        if (is_file($file_absolute)) {
            $length = filesize($file_absolute);
        } elseif ($content != '') {
            $length = strlen($content);
        } else {
            throw new Exception($file_absolute . '下载文件不存在！');
        }
        if (empty($show_name)) {
            $show_name = basename($file_absolute);
        }
        if (!empty($filename)) {
