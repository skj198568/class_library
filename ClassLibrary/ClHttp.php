...
class ClHttp {
     ...
     * @param array $post 数组形式
     * @param string $cookie 携带cookie访问,该参数是cookie内容
     * @param string $content_type header type
     * @param int $timeout 采集超时时间
     * @param int $limit 分段读取字符个数
     * @param bool $block 是否阻塞访问,默认为true
     * @return string
     */
    public static function fsockopenDownload($url, $post = [], $cookie = '', $content_type = 'application/x-www-form-urlencoded', $timeout = 15, $limit = 0, $block = true) {
        $return  = '';
        $matches = parse_url($url);
        !isset($matches['host']) && $matches['host'] = '';
        !isset($matches['path']) && $matches['path'] = '';
        ...
        $path       = $matches['path'] ? $matches['path'] . ($matches['query'] ? '?' . $matches['query'] : '') : '/';
        $port       = !empty($matches['port']) ? $matches['port'] : 80;
        $referer    = $matches['scheme'] . '://' . $matches['host'];
